...
- **Query factory pattern.** Query options exported as hook functions (`useXxxQuery`) calling `useQuery` or `useInfiniteQuery` directly. Components call `useXxxQuery(...)` — no wrapping at call site.
...
| Client data | TanStack React Query | Server-state for admin CRUD pages. Query options exported as factory functions. Infinite queries for cursor-paginated lists. Mutations check unified response `success` field. Query invalidation in `onSettled` |
...
frontend/src/features/image/
  ├── queries/ — useQuery + useInfiniteQuery hooks
