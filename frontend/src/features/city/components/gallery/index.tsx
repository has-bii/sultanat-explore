...
export function CityGallery({ cityId }: Props) {
  const { data } = useSuspenseQuery(getCityGalleryQueryOptions(cityId))
  const initialImages = data.map(({ image }) => image)
  const syncMutation = useUpdateGallery(cityId)
  ...
  return (
    <Card className="w-full">
      <CardHeader className="border-b">
        <CardTitle>Galeri Kota</CardTitle>
